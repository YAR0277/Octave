function [] = State(this,state)
  ...
  for i=1:length(idx)
    fprintf('index(%d), district(%s), RR_sphere(%.4f), RR_spheroid(%.4f), RR_gate(%.4f)\n',...
      idx(i),T.dname{idx(i),1},T.RR_sphere(idx(i)),T.RR_spheroid(idx(i)),T.RR_gate(idx(i)));
  endfor
  ...
  fprintf('RR_gate: range([%.4f,%.4f]), mean(%.4f)\n',min(T.RR_gate(idx)),max(T.RR_gate(idx)),mean(T.RR_gate(idx)));
endfunction
